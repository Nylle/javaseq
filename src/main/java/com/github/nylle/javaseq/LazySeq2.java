...
class LazySeq2<T> extends ASeq<T> implements ISeq<T> {
    ...
    public ISeq<T> filter(Predicate<? super T> pred) {
        ...
        if (pred.test(first())) {
            return ISeq.cons(first(), ISeq.lazySeq2(() -> rest().filter(pred)));
        } else {
            return ISeq.lazySeq2(() -> rest().filter(pred));
        }
    }
    ...
    public <R> ISeq<R> map(Function<? super T, ? extends R> f) {
        ...
        return ISeq.cons(f.apply(first()), ISeq.lazySeq2(() -> rest().map(f)));
    }
    ...
    public <S, R> ISeq<R> map(ISeq<? extends S> coll, BiFunction<? super T, ? super S, ? extends R> f) {
        return seq().isEmpty() || coll.isEmpty()
                ? ISeq.of()
                : ISeq.cons(f.apply(first(), coll.first()), ISeq.lazySeq2(() -> rest().map(coll.rest(), f)));
    }
    ...
    public <R> ISeq<R> mapcat(Function<? super T, ? extends Iterable<? extends R>> f) {
        return seq().isEmpty()
                ? ISeq.of()
                : concat(ASeq.<R>copy(f.apply(first())).iterator(), ISeq.lazySeq2(() -> rest().mapcat(f)));
    }
    ...
    public <S, R> ISeq<R> mapcat(ISeq<? extends S> coll, BiFunction<? super T, ? super S, Iterable<? extends R>> f) {
        return seq().isEmpty() || coll.isEmpty()
                ? ISeq.of()
                : concat(ASeq.<R>copy(f.apply(first(), coll.first())).iterator(), ISeq.lazySeq2(() -> rest().mapcat(coll.rest(), f)));
    }
    ...
    public ISeq<List<T>> partition(int n, int step, Iterable<T> pad) {
        ...
        if (partition.size() < n) {
            return pad == null
                    ...
                    ISeq.lazySeq2(() -> drop(step).partition(n, step, pad)));
        }
        return ISeq.cons(partition, ISeq.lazySeq2(() -> drop(step).partition(n, step, pad)));
    }
    ...
    public <U> ISeq<U> reductions(U init, BiFunction<U, ? super T, U> f) {
        ...
        return ISeq.cons(init, ISeq.lazySeq2(() -> rest().reductions(f.apply(init, first()), f)));
    }
    ...
    private static <T> ISeq<T> step(ISeq<T> seq, Set<T> seen) {
        ...
        return ISeq.cons(first, ISeq.lazySeq2(() -> step(result.rest(), Util.conj(seen, first))));
    }
